...
BigNumber SunsRightAscension(const BigNumber &epsilon, const BigNumber &lambda)
{
    ...
    if (calculationSpeed == SLOW_ACCURATE) {
        ...
        if (region(BigNumberMath::to_BigRad(lambda)))
            v3 = BigNumberMath::minusOne * (BigNumberMath::one - v2 * v2).sqrt();
        else
            v3 = (BigNumberMath::one - v2 * v2).sqrt();
...
void findAlt(const BigNumber &latitude, const BigNumber &delta, const BigNumber &h)
{
    ...
    if (calculationSpeed == SLOW_ACCURATE) {
        ...
        // trick to calculate cosine
        if (region(BigNumberMath::to_BigRad(v6)))
            v3 = BigNumberMath::minusOne * (BigNumberMath::one - v1 * v1).sqrt();
        else
            v3 = (BigNumberMath::one - v1 * v1).sqrt();

        //v4 = BigNumberMath::cosine(BigNumberMath::to_BigRad(delta));  // sj
        // trick to calculate cosine
        if (region(BigNumberMath::to_BigRad(delta)))
            v4 = BigNumberMath::minusOne * (BigNumberMath::one - v2 * v2).sqrt();
        else
            v4 = (BigNumberMath::one - v2 * v2).sqrt();
...
BigNumber findAzSubFunction(const BigNumber &latitude, const BigNumber &delta, const BigNumber &h)
{
    ...
    if (calculationSpeed == SLOW_ACCURATE) {
        ...
        // trick to calculate cosine
        if (region(BigNumberMath::to_BigRad(delta)))
            v8 = BigNumberMath::minusOne * (BigNumberMath::one - v7 * v7).sqrt();
        else
            v8 = (BigNumberMath::one - v7 * v7).sqrt();

        //v5 = BigNumberMath::cosine(BigNumberMath::to_BigRad(latitude));  // sj
        // trick to calculate cosine
        if (region(BigNumberMath::to_BigRad(latitude)))
            v5 = BigNumberMath::minusOne * (BigNumberMath::one - v3 * v3).sqrt();
        else
            v5 = (BigNumberMath::one - v3 * v3).sqrt();
    }
    v4 = v7 / v8;
    v7 = ((v2 * v3) - v4 * v5);
    return v7;
